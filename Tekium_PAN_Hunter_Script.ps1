...
write-host "Tekium PAN Hunter Script" -ForegroundColor green
Write-Host "-------------------------------------------------------------------------------------" -ForegroundColor Yellow
Write-Host "Hostname: $hostname"
Write-Host ""
Write-Host "IP Address: $ip_address"
Write-Host ""
$prueba = Get-ChildItem -Path $path_search -Recurse -include $filters | Select-String -Pattern $regex_amex, $regex_visa, $regex_master -AllMatches | Select-Object Filename, Line
if ( $prueba )
{
    ...
    "Tekium PAN Hunter Script" | Out-File -FilePath $log -Append
    "-------------------------------------------------------------------------------------" | Out-File -FilePath $log -Append
